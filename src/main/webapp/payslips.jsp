            ...
            <div class="dashboard-container">
                ...
                <div class="filter-card" style="margin-top: 16px;">
                    ...
                    <form method="post" action="payslips" style="display: flex; gap: 16px; align-items: center;">
                        ...
                        <!-- Employé -->
                        <div>
                            <input list="employeesList" name="user" class="input" value="<%= request.getAttribute("filter_user") %>" placeholder="ex: EMP123" oninput="checkEmployeeValid(this)" data-required="false" style="background: rgba(255, 255, 255, 0.1); color: white; border: 1px solid rgba(255, 255, 255, 0.3); border-radius: 10px; padding: 10px; font-size: 1rem; backdrop-filter: blur(8px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3); width: 200px;">
                        ...
                        <!-- Année -->
                        <div>
                            <input name="year" class="input" type="number" value="<%= request.getAttribute("filter_year") %>" placeholder="ex : 2024" style="background: rgba(255, 255, 255, 0.1); color: white; border: 1px solid rgba(255, 255, 255, 0.3); border-radius: 10px; padding: 10px; font-size: 1rem; backdrop-filter: blur(8px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3); width: 120px;">
                        </div>

                        <!-- Mois -->
                        <div>
                            <select name="month" class="input" style="background: rgba(255, 255, 255, 0.1); color: white; border: 1px solid rgba(255, 255, 255, 0.3); border-radius: 10px; padding: 10px; font-size: 1rem; backdrop-filter: blur(8px); box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3); width: 150px;">
                        ...
                        <!-- Bouton Filtrer -->
                        <button class="welcome-logout" style="background: linear-gradient(135deg, #3b82f6, #06b6d4); color: #e0f2fe; font-weight: 600; border-radius: 10px; padding: 10px 14px; box-shadow: 0 8px 22px rgba(59,130,246,.35); border: none; cursor: pointer; font-size: 1rem; transition: filter 0.2s ease-in-out;">Filtrer</button>
                    </form>

                    <!-- Conteneur des boutons Filtrer et Réinitialiser (Alignés horizontalement) -->
                    <div style="display: flex; gap: 16px; margin-top: 10px;">
                        <!-- Bouton Réinitialiser dans un formulaire séparé -->
                        <form method="post" action="payslips" style="display:inline;">
                            <input type="hidden" name="action" value="reset">
                            <input type="hidden" name="csrfToken" value="<%= request.getAttribute("csrfToken") %>">
                            <button class="welcome-logout" style="background: #ef4444; color: #fff; font-weight: 600; border-radius: 10px; padding: 10px 14px; box-shadow: 0 8px 22px rgba(239, 68, 68, .35); border: none; cursor: pointer; font-size: 1rem; transition: filter 0.2s ease-in-out;">Réinitialiser</button>
                        </form>
                    </div>
                </div>
...
<div id="modalUpdate" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.6); z-index:999; justify-content:center; align-items:center; overflow:auto;">
        ...
        <form method="post" action="payslips" id="updateForm">
            ...
            <div style="display: flex; justify-content: space-between; gap: 10px;">
                ...
                <button type="button" class="welcome-logout"
                        style="background: #ef4444; color: #fff; font-weight: 600; border-radius: 10px; padding: 10px 14px; box-shadow: 0 8px 22px rgba(239, 68, 68, .35); border: none; cursor: pointer; font-size: 1rem; transition: filter 0.2s ease-in-out;" onclick="togglePayslipModal(null)">
                    Annuler
                </button>
            </div>
        </form>
